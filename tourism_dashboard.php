  ...
  <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 md:w-1/2 lg:w-1/3 relative modal-content">
    ...
    <form id="messageForm">
        ...
        <div class="flex justify-end space-x-2">
            <button type="button" onclick="closeMessageModal()" class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">Cancel</button>
            <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded">Send</button>
        </div>
    </form>
  </div>
</div>
...
<div id="logoutModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center z-50">
  <div class="bg-white p-6 rounded shadow-lg w-80">
    ...
    <div class="flex justify-end space-x-3">
      ...
      <a href="admin_login.php" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Logout</a>
    </div>
  </div>
</div>
...
<script>
let spotsData = <?= json_encode($spots) ?>;
let currentMap = null;
...
function openModal(id){
    ...
    document.getElementById('modalPostedBy').innerText = spot.posted_by_name ?? 'N/A';
    document.getElementById('modalDescription').innerText = spot.description;
    document.getElementById('modalFee').innerText = spot.entrance_fee ? '₱'+spot.entrance_fee : 'N/A';
    ...
    currentMap = L.map('modalMap').setView([parseFloat(spot.latitude)||0, parseFloat(spot.longitude)||0], 15);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'&copy; OpenStreetMap contributors'}).addTo(currentMap);
    L.marker([parseFloat(spot.latitude)||0, parseFloat(spot.longitude)||0]).addTo(currentMap).bindPopup(spot.name_of_tourist_spot).openPopup();
    ...
    if(['pending','modified'].includes(spot.status.toLowerCase())){
        ...
        approve.className='px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700';
        approve.onclick=()=>updateSpotStatus(spot.id,'verify');

        const reject=document.createElement('button');
        reject.innerText='Reject';
        reject.className='px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700';
