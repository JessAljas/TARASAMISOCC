...
function initSpotMap(lat = 8.15, lng = 123.85, spotName = "Tourist Spot") {
    ...
    }).addTo(map).bindPopup(`<b>${spotName}</b>`).openPopup();
}
...
function openPackageModal(id){
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        div.innerHTML = `${img?`<img src="uploads/${img}" class="w-24 h-24 object-cover mb-1 border rounded">`:''}
                         <input type="file" name="edit_images[${i-1}]" class="w-full">`;
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        container.innerHTML += `
          <div class="mb-2">
              ...
              ${img ? `<img src="uploads/${img}" class="w-24 h-24 object-cover mb-2 border rounded">` : ''}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof spotLatitude !== 'undefined' && typeof spotLongitude !== 'undefined') {
